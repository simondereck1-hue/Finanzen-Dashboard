...
with tabs[3]:
    ...
    if not filtered_ausgaben.empty:
        ...
        with col2:
            ...
            if not disp_df.empty and 'Datum' in disp_df.columns: disp_df['Datum'] = disp_df['Datum'].dt.strftime('%d.%m.%Y')
...
with tabs[4]:
    ...
    if not df_ausgaben.empty or not df_einnahmen.empty:
        ...
        for d in [df_v, df_e]:
            if 'Datum' in d.columns and d['Datum'].notnull().any(): d['Monat_Sort'] = d['Datum'].dt.strftime('%Y-%m')
        ...
        df_ausg_all = pd.concat([(df_fixkosten.copy().assign(Betrag=df_fixkosten['Betrag']*num_months)).assign(Quelle='Fixkosten'), filtered_ausgaben.assign(Quelle='Variable Ausgaben')])
        ...
        if not df_ausg_all.empty or not total_einn_fix.empty:
            ...
            fig_sankey = go.Figure(data=[go.Sankey(node=dict(pad=15, thickness=20, label=label_list, color="lightgray"), link=dict(source=source, target=target, value=value, color=color_link))])
    ...
    if not df_ausgaben.empty:
        ...
        with trend_tabs[0]:
            ...
            with col_plot:
                ...
                if not df_tk.empty and df_tk['Datum'].notnull().any():
                    df_tk['Monat_Sort'] = df_tk['Datum'].dt.strftime('%Y-%m')
                    df_tk['Monat'] = df_tk['Datum'].apply(lambda x: f"{MONATE_DE[x.strftime('%m')]} {x.year}")
        ...
        with trend_tabs[1]:
            ...
            with col_plot:
                ...
                if not df_ts.empty and df_ts['Datum'].notnull().any():
                    df_ts['Monat_Sort'] = df_ts['Datum'].dt.strftime('%Y-%m')
                    df_ts['Monat'] = df_ts['Datum'].apply(lambda x: f"{MONATE_DE[x.strftime('%m')]} {x.year}")
    ...
    with tabs[6]:
        ...
        if akt_einn > 0:
            ...
            for d in [df_v_all, df_e_all]: 
                if 'Datum' in d.columns: d['Monat_Sort'] = d['Datum'].dt.strftime('%Y-%m')
